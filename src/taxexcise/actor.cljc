(ns taxexcise.actor
  "TaxExciseActor — the ISCO-08 3352 Government Tax and Excise
  Officials documentation/logistics-coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one filing-documentation or logistics
  ...
  The unconditional invariant: the TaxFilingAdvisor can never directly
  commit a record the TaxExciseGovernor refuses — every commit-record!
  call is gated behind `:decide`. Neither the advisor nor this graph
  can ever finalize a tax assessment, impose a penalty, or order a
  collection/lien action (see taxexcise.governor's
  no-enforcement-authority guarantee docstring); every observation
  that might warrant one is only ever surfaced via
  :flag-compliance-concern, which always routes through
  :request-approval for a human tax official."
...
(defn build-graph
  "Build a compiled TaxExciseActor graph. `store` implements
  `taxexcise.store/Store`. `advisor` implements
  `taxexcise.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:filer-id (:filer-id request)
                                    :office-id (:office-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
